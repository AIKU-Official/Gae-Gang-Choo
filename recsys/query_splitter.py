...
class QuerySplitter:
  ...
  def _create_prompt_template(self):
    ...
    prompt_template = PromptTemplate(
        ...
        partial_variables = {'example_query': example_query, 'example_answer': example_answer}
        )
  ...
  def _get_model_input(self, query):
    ...
    conversation = [
        {"role": "system", "content": "너는 문장을 '수업의 강의 주제과 관련된 문장', '수업의 평가와 관련된 문장'으로 분류하는 문장 분류 전문가야."},
  ...
  def _postprocess_output(self, output_text, inst_token='[/INST]', eos_token='</s>'):
    output_text = output_text[output_text.find(inst_token) + len(inst_token):output_text.find(eos_token)]
    output_text = output_text[output_text.find('{'):output_text.find('}') + 1]

    output_text = output_text.replace(' "', '"') # Tokenizer때문에 공백이 더해지는 현상 해결
    return output_text

  def split(self, query):
    prompt = self._get_model_input(query)

    max_new_tokens = len(self.tokenizer(query, return_tensors="pt", add_special_tokens=False).input_ids[0]) + 32
