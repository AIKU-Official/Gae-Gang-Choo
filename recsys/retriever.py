...
class ReciprocalRetriever:
    ...
    def query(
      ...
      course_ids = [list(map(lambda x: x['course_id'], i)) for i in result['metadatas']]
      ...
      for query_i in range(len(relevances)):
        ...
        while (not queue.empty()) and (len(supporting_id_over_query.values()) < topk):
          relevance, course_id, id = queue.get()
          rel_over_query[course_id].append(relevance)
          supporting_id_over_query[course_id].append(id)

        for key in rel_over_query:
          rel_over_query[key] = sum(rel_over_query[key]) / len(rel_over_query[key])
      ...
      for key in course_score.keys():
        course_score[key] = (sum(course_score[key]) / len(query_texts)) if course_score[key] else 0

      course_id, course_score = zip(*sorted(course_score.items(), key = lambda x: x[1], reverse=True))
